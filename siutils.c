...
static void si_gci_chipctrl_overrides(osl_t *osh, si_t *sih, char *pvars);
...
#if defined(BCMSDIO) && (defined(BCMDONGLEHOST) || defined(BCMDBUS)) && \
	!defined(BCMSDIOLITE)
...
BCMATTACHFN(si_muxenab)(si_t *sih, uint32 w)
{
	...
	CASE_BCM43602_CHIP:
		...
		break;
	CASE_BCM4345_CHIP:
		/* these names look whacky..why is there a ref to 4335 */
	...
	case BCM43012_CHIP_ID:
		...
		if (w & MUXENAB43012_HOSTWAKE_MASK) {
			...
			si_gci_set_functionsel(sih, hostwake, CC43012_FNSEL_MISC1);
		}
		break;
		...
		*/
		if (w & MUXENAB4335_UART_MASK) {
			...
			if (uartopt_ix >
				...
				SI_ERROR(("si_muxenab: wrong index %d for uart\n",
					uartopt_ix));
				break;
			}
	...
	case BCM4362_CHIP_GRPID:
		...
		if (w & MUXENAB_HOST_WAKE) {
			si_gci_set_functionsel(sih, CC4362_PIN_GPIO_00, CC4362_FNSEL_MISC1);
		}
		break;
...
si_gpio_enable(si_t *sih, uint32 mask)
{
	...
	} else if ((BCM4347_CHIP(sih->chip)) || BCM4369_CHIP(sih->chip) ||
	           BCM4362_CHIP(sih->chip)) {
		fnsel = CC4347_FNSEL_SAMEASPIN;
	} else if (BCM4378_CHIP(sih->chip)) {
		fnsel = CC4378_FNSEL_SAMEASPIN;
	} else if (BCM4387_CHIP(sih->chip)) {
		fnsel = CC4387_FNSEL_SAMEASPIN;
	}
	...
	*/
	...
	if (!FWSIGN_ENAB()) {
		if (HIB_EXT_WAKEUP_CAP(sih)) {
			...
			if (getintvar(NULL, rstr_ext_wakeup_dis)) {
				...
				/* Disable now until the GPIO false wakeup issue is solved */
				sii->hib_ext_wakeup_enab = FALSE;
	...
	if (!si_onetimeinit) {
		...
		if (!FWSIGN_ENAB()) {
			...
			/* Handle ccidiv (cpu_clock:cci_clock) ratio from NVRAM file */
			if (BCM4368_CHIP(sih->chip)) {
				if ((val = getvar(NULL, rstr_ccidiv)) != NULL) {
					sii->ccidiv = (uint32) bcm_strtoul(val, NULL, 0);
					if (sii->ccidiv > CCIDIV_4_TO_1) {
						ASSERT(0);
					}
				} else {
					sii->ccidiv = CCIDIV_4_TO_1;
				}
			}
#endif /* DONGLEBUILD */
		}
...
#ifdef BCMQT
			/* In the QT setup looks like LPO is not enabled???? */
			/* may be only for PCIE pkgs */
	...
	/* Not for 4335, otherwise DVT has to be run again */
...
si_sdio_init(si_t *sih)
{
	...
	if (((BUSCORETYPE(sih->buscoretype) == PCMCIA_CORE_ID) && (sih->buscorerev >= 8)) ||
	    (BUSCORETYPE(sih->buscoretype) == SDIOD_CORE_ID)) {
		uint idx;
	...
	if (corerev >= 10) {
		...
		for (i = 0; i < nb; i++) {
			...
			if (set) {
				...
				/* setting of enable/protect are global for all banks */
...
static chipcregs_t *
si_seci_access_preamble(si_t *sih, const si_info_t *sii, uint32 *origidx, bool *fast)
{
	chipcregs_t *cc = seci_set_core(sih, origidx, fast);
...
si_cis_source(const si_t *sih)
{
	...
	case BCM4368_CHIP_ID:
		return CIS_OTP;	/* revisit if sprom_present chipstatus bit becomes available */
	case BCM4378_CHIP_GRPID:
...
#if !defined(BCMDONGLEHOST)
uint32
si_pcie_set_ctrlreg(const si_t *sih, uint32 mask, uint32 val)
{
#if (!defined(BCMDONGLEHOST) && !defined(BCMSDIO))
	const si_info_t *sii = SI_INFO(sih);

	if (!PCIE(sii))
		return 0;

	/* SWWLAN-33436 : sometimes system will get the wrong sub system ID */
...
#endif /* (!defined(BCMDONGLEHOST) && !defined(BCMSDIO)) */
	return 0;
}
#endif /* !defined(BCMDONGLEHOST) */
...
si_slave_wrapper_add(si_t *sih)
{
	...
	/* HW4345-1251 - WAR for incorrect EROM base
...
void
si_introff(const si_t *sih, bcm_int_bitmask_t *intr_val)
{
	const si_info_t *sii = SI_INFO(sih);
	INTR_OFF(sii, intr_val);
}
...
void
si_intrrestore(const si_t *sih, bcm_int_bitmask_t *intr_val)
{
	const si_info_t *sii = SI_INFO(sih);
	INTR_RESTORE(sii, intr_val);
}
...
void
si_nvram_rfldo3p3_war(const si_t *sih, uint32 *min_mask)
{
	const si_info_t *sii = SI_INFO(sih);
	if (sii->rfldo3p3_war == TRUE) {
...
uint32
si_get_armpllclkfreq(const si_t *sih)
{
	const si_info_t *sii = SI_INFO(sih);
...
uint32
si_wrapper_dump_binary(const si_t *sih, uchar *p)
{
	if (CHIPTYPE(sih->socitype) == SOCI_AI)
...
uint32
si_wrapper_dump_last_timeout(const si_t *sih, uint32 *error, uint32 *core, uint32 *ba, uchar *p)
{
	if (CHIPTYPE(sih->socitype) == SOCI_AI)
...
#endif /* (BCM_BACKPLANE_TIMEOUT) || (AXI_TIMEOUTS) */
	return;
}
...
bool
si_check_enable_backplane_log(const si_t *sih)
{
	if (CHIPTYPE(sih->socitype) == SOCI_AI) {
...
#ifndef BCMDONGLEHOST
#define RF_SW_CTRL_OVERRIDE_MASK	0x01FFFFFF
#define RF_SW_CTRL_OVERRIDE_ENABLE	0x00F0C030
#define RF_SW_CTRL_OVERRIDE_DEFAULT_2G	0x00300000
#define RF_SW_CTRL_OVERRIDE_DEFAULT_5G	0x00C00000
#define RF_SW_CTRL_ELNABYP_ANT_MASK	0x0000C030
...
#define ELNABYP_IOVAR_2G0_MASK		0x01
#define ELNABYP_IOVAR_5G0_MASK		0x02
#define ELNABYP_IOVAR_2G1_MASK		0x04
#define ELNABYP_IOVAR_5G1_MASK		0x08
...
si_rffe_rfem_init(si_t *sih)
{
	...
	/* reg address = 0x16, deviceID of rffe dev1 = 0xD, deviceID of dev0 = 0xC,
	 * last_mux_ctrl = 0, disable_preemption = 1, tssi_mask = 3, tssi_en = 0,
	 * rffe_disable_line1 = 0, enable rffe_en_phyaccess = 1,
	 * disable BRCM proprietary reg0 wr = 0
	 */
	si_gci_direct(sih, OFFSETOF(gciregs_t, gci_rffe_misc_ctrl), ALLONES_32, 0x0016DC72);

	/* Assert VIO to all 2G rFEMs */
	/* Set rf_sw_ctrl_override_en */
	si_gci_chipcontrol(sih, CC_GCI_CHIPCTRL_15, RF_SW_CTRL_OVERRIDE_MASK,
		RF_SW_CTRL_OVERRIDE_DEFAULT_2G);
	/* Set rf_sw_ctrl_override_value */
	si_gci_chipcontrol(sih, CC_GCI_CHIPCTRL_14, RF_SW_CTRL_OVERRIDE_MASK,
		RF_SW_CTRL_OVERRIDE_DEFAULT_2G);

	/* Delay to power up RFEMs */
	OSL_DELAY(2000);

	/* Enable RFEM Register Access for both Antennas (0 and 1) */
	si_rffe_rfem_write(sih, 0, 3, 0x1C, 0x0);
	/* Change dev_id from 0xE to 0xC for 2G RFEMs */
	si_rffe_rfem_write(sih, 0xE, 3, 0x1F, 0xC);

	/* Assert VIO to all rFEMs */
	/* Set rf_sw_ctrl_override_en */
	si_gci_chipcontrol(sih, CC_GCI_CHIPCTRL_15, RF_SW_CTRL_OVERRIDE_MASK,
		RF_SW_CTRL_OVERRIDE_DEFAULT_2G | RF_SW_CTRL_OVERRIDE_DEFAULT_5G);
	/* Set rf_sw_ctrl_override_value */
	si_gci_chipcontrol(sih, CC_GCI_CHIPCTRL_14, RF_SW_CTRL_OVERRIDE_MASK,
		RF_SW_CTRL_OVERRIDE_DEFAULT_2G | RF_SW_CTRL_OVERRIDE_DEFAULT_5G);
	...
	/* Enable RFEM Register Access for both Antennas (0 and 1) */
	si_rffe_rfem_write(sih, 0xE, 3, 0x1C, 0x0);
}
...
si_rffe_set_debug_mode(si_t *sih, bool enable)
{
	...
	if (enable) {
		/* Set rf_sw_ctrl_override_en */
		si_gci_chipcontrol(sih, CC_GCI_CHIPCTRL_15, RF_SW_CTRL_OVERRIDE_MASK,
			RF_SW_CTRL_OVERRIDE_ENABLE);
		sih->rffe_debug_mode =  TRUE;
	} else {
		/* Set rf_sw_ctrl_override_en */
		si_gci_chipcontrol(sih, CC_GCI_CHIPCTRL_15, RF_SW_CTRL_OVERRIDE_MASK,
			RF_SW_CTRL_OVERRIDE_DEFAULT_2G | RF_SW_CTRL_OVERRIDE_DEFAULT_5G);
		sih->rffe_debug_mode =  FALSE;
	}
}
...
{
	int ret = BCME_OK;
	uint32 elnabyp_val = 0;

	if (mode > 0xF) {
		ret = BCME_BADOPTION;
	} else {
		if (mode & ELNABYP_IOVAR_2G0_MASK) {
			elnabyp_val |= RF_SW_CTRL_ELNABYP_2G0_MASK;
		}
		if (mode & ELNABYP_IOVAR_5G0_MASK) {
			elnabyp_val |= RF_SW_CTRL_ELNABYP_5G0_MASK;
		}
		if (mode & ELNABYP_IOVAR_2G1_MASK) {
			elnabyp_val |= RF_SW_CTRL_ELNABYP_2G1_MASK;
		}
		if (mode & ELNABYP_IOVAR_5G1_MASK) {
			elnabyp_val |= RF_SW_CTRL_ELNABYP_5G1_MASK;
		}
...
int
si_rffe_rfem_read(si_t *sih, uint8 dev_id, uint8 antenna, uint16 reg_addr, uint32 *val)
{
	int ret = BCME_OK;
	uint32 gci_rffe_ctrl_val, antenna_0_enable, antenna_1_enable;
	uint32 regval = si_gci_direct(sih, OFFSETOF(gciregs_t, gci_rffe_ctrl), 0, 0);
	...
	si_gci_direct(sih, OFFSETOF(gciregs_t, gci_rffe_ctrl), ALLONES_32, regval);
	return ret;
}

int
si_rffe_rfem_write(si_t *sih, uint8 dev_id, uint8 antenna, uint16 reg_addr, uint32 data)
{
	int ret = BCME_OK;
	...
	if (ret == BCME_OK) {
		...
		/* Initiate write */
		si_gci_direct(sih, OFFSETOF(gciregs_t, gci_rffe_ctrl),
			RFFE_CTRL_START | RFFE_CTRL_READ | RFFE_CTRL_RFEM_SEL, RFFE_CTRL_START);
		/* Wait for status */
		SPINWAIT(si_gci_direct(sih, OFFSETOF(gciregs_t, gci_rffe_ctrl), 0, 0) &
			RFFE_CTRL_START, 100);
		if (si_gci_direct(sih, OFFSETOF(gciregs_t, gci_rffe_ctrl), 0, 0) &
