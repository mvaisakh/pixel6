...
#define PLL(_id, cname, _type, lock, con, en, enbit,		\
		sel, selbit, stat, statbit, rtable, pf, f, a)	\
	{							\
		.id		= _id,				\
		.name		= cname,			\
		.type		= _type,			\
...
#define MUX(_id, cname, pnames, sel, selbit,			\
		selwid, stat, statbit, statwid, f, a)		\
	{							\
		.id		= _id,				\
		.name		= cname,			\
		.parents	= pnames,			\
...
#define DIV(_id, cname, pname, rate, ratebit,			\
		ratewid, stat, statbit, statwid, f, a)		\
	{							\
		.id		= _id,				\
		.name		= cname,			\
		.parent_name	= pname,			\
...
extern void samsung_register_comp_divider(struct samsung_clk_provider *ctx,
				struct samsung_composite_divider *div_list,
				unsigned int nr_div);
...
#define HWACG_VCLK_QACTIVE(_id, _name, _parent, f, vf,		\
				a, _addr, _mask, _val)		\
	{							\
		.id		= _id,				\
		.name		= _name,			\
